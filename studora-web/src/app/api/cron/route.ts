...
import { db } from "@/db";
import { assignment, assignmentRoomMember, assignmentSubmission, user, assignmentRoom } from "@/db/schema";
import { lt, and, eq, gte } from "drizzle-orm";
import nodemailer from "nodemailer";

export async function GET(request: Request) {
  try {
    // Vercel Cron sends a CRON_SECRET header or we can verify query param
    const authHeader = request.headers.get('authorization');
    const cronSecret = process.env.CRON_SECRET;
    
    // In local dev, we might not have CRON_SECRET, but in prod we must
    if (cronSecret && authHeader !== `Bearer ${cronSecret}`) {
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }

    console.log("[Cron] Running expired assignment cleanup & reminder job via API...");
    ...
    for (const asm of upcoming24h) {
      ...
      console.log(`[Cron API] Sent 24h reminder for assignment ${asm.id}`);
    }
