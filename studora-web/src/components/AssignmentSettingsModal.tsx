...
import { renameAssignmentRoomAction, transferAssignmentOwnership, deleteAssignmentRoomAction } from "@/actions/assignment";
...
export default function AssignmentSettingsModal({ isOpen, onClose, roomId, roomName, members }: RoomSettingsProps) {
  ...
  const handleRename = async (e: React.FormEvent) => {
    ...
    if (newName !== roomName) {
      const res = await renameAssignmentRoomAction(roomId, newName);
  ...
  const handleTransfer = async (e: React.FormEvent) => {
    ...
    const res = await transferAssignmentOwnership(roomId, targetUserId);
    if (res.error) setError(res.error);
  ...
  const handleDelete = async (e: React.FormEvent) => {
    ...
    const res = await deleteAssignmentRoomAction(roomId);
    if (res.error) setError(res.error);
    else {
