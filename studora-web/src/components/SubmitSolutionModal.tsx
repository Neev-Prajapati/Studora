...
import { submitAssignmentSolutionAction } from "@/actions/assignment";
...
export default function SubmitSolutionModal({ isOpen, onClose, roomId, assignment }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      uploadData.append("roomId", roomId); // Re-using folder structure
      uploadData.append("roomType", "assignment");
      uploadData.append("uploadType", "solution");
      ...
      const res = await submitAssignmentSolutionAction(
        assignment.id,
        roomId,
        uploadJson.fileUrl,
        uploadJson.fileName
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
      <div className="w-full max-w-md bg-card border border-border rounded-xl shadow-lg relative overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-foreground mb-1">Your Solution File <span className="text-destructive">*</span></label>
            <input 
              name="file" 
              type="file" 
              required 
            ...
            <p className="text-xs text-muted-foreground mt-2">
              Note: Submitting again will overwrite your previous submission for this assignment.
            </p>
          </div>
